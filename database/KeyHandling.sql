...
CREATE TABLE IF NOT EXISTS "_System_KeyStore" (
...
REVOKE ALL ON "_System_KeyStore" FROM PUBLIC;

-- Grant access ONLY to the specific superuser (usually 'postgres')
-- Change 'postgres' to your actual root username if different.
GRANT ALL ON "_System_KeyStore" TO postgres;

-- 3. INSERT YOUR KEYS (Run this ONCE)
-- ⚠️ WARNING: In a real Prod Env, these should be in Azure KeyVault/AWS KMS.
INSERT INTO "_System_KeyStore" ("KeyName", "KeyValue", "Description")
VALUES 
    ('app.enc_key',   'YOUR_STRONG_AES_KEY_32_CHARS_LONG!!', 'Used for AES Encryption (User_Secrets)'),
...
BEGIN
    ...
    -- Fetch Keys
    SELECT "KeyValue" INTO v_enc FROM "_System_KeyStore" WHERE "KeyName" = 'app.enc_key';
    SELECT "KeyValue" INTO v_blind FROM "_System_KeyStore" WHERE "KeyName" = 'app.blind_key';
