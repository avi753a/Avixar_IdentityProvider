 ...
 *   - Pseudonymization: Yes (Separated from users).
 ...
 * 
 * 1. Login Query:
 *    "SET app.blind_key = @key; SELECT * FROM users WHERE Email_Hash = encode(hmac(@email, current_setting('app.blind_key'), 'sha256'), 'hex');"
 *
 * 2. Create User:
 *    "SET app.enc_key = @k1; SET app.blind_key = @k2; CALL sp_CreateUser(...);"
 *
 * 3. Decrypt for Email Sending:
 *    "SET app.enc_key = @key; SELECT pgp_sym_decrypt(Email_Enc, current_setting('app.enc_key')) FROM users WHERE Id = @uid;"
 */